		...
		if(rcvd < 0) clean_exit(rc, server_fd, "[Server recv error]: ");
			...
			if (strncmp(reqline[0], "GET\0", 4) == 0){
				...
				if(strncmp(reqline[2], "HTTP/1.0", 8) != 0){
					...
					else if(!(strstr(reqline[1], ".html")) && 
						!(strstr(reqline[1], ".js")) && 
						!(strstr(reqline[1], ".css")) && 
						!(strstr(reqline[1], ".txt")) && 
						!(strstr(reqline[1], ".jpeg")))
					{
						write(client_fd, "HTTP1.0 400 Bad Request\n", 25);
					}
					else if(strstr(reqline[3], "If-Modified-Since")){
						//Conditional GET has been found
						printf("Whoo Woo!\n");
						...
						else write(client_fd, "HTTP1.0 404 Not Found\n", 23);	
					}
				}
			}
